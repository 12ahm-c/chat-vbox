...
زميلتي تعمل على اعداد components drivers و لقد قامت بالتغييرات و العمل الخاص به. و رفعته في branch الخاص بها تصفحته فجمعت لك اكواد بعض الملفات التي وجدتها أجرتهم فيهم تغيير ما من بين ملفات الذي رقتهم أنا في main 

سارسل لك أكوادها و انت تصلحها ان كان فيها عطل و تدمجها داخل المشروع حتى تعمل components drivers التي تعمل عليها زميلتي هذاه و تعمل و تتفاعل مع المشروع دون اي خلل و إياك ان تفسد المشروع 
...
class DriverCubit extends Cubit<DriverState> {
  ...
  void loadDriver(String driverId) async {
    emit(DriverLoading());

    // Register FCM Token
    try {
      final pushService = PushNotificationService();
      await pushService.updateToken(driverId, 'driver');
    } catch (e) {
      if (kDebugMode) print('Error updating FCM token: $e');
    }

    // Fetch initial wallet balance FIRST before anything else
    final initialBalance = await _driverService.getWalletBalance(driverId);
    
    // Auto-create wallet document if it doesn't exist (fixes legacy drivers missing wallets)
    await _driverService.ensureWalletExists(driverId, initialBalance);

    // Start GPS tracking right away
    _startLocationTracking(driverId);

    // Notifications will be listened to after profile is loaded

    _profileSubscription?.cancel();
    _profileSubscription = _driverService.streamDriverModel(driverId).listen(
      (profile) {
        if (state is DriverLoaded) {
          final prev = state as DriverLoaded;
          // Preserve the wallet balance from the previous state since it's managed by streamWalletBalance
          final updatedProfile = profile.copyWith(walletBalance: prev.profile.walletBalance);
          emit(prev.copyWith(profile: updatedProfile));
        } else {
          // First load: use the initial wallet balance we fetched
          final profileWithBalance = profile.copyWith(walletBalance: initialBalance);
          emit(DriverLoaded(profile: profileWithBalance));
        }

        // Use the current state's profile (which has real balance) for the check
        if (state is DriverLoaded) {
          final currentProfile = (state as DriverLoaded).profile;
          if (currentProfile.canReceiveRequests) {
            _listenToRequests();
          } else {
        ...
        // Always listen to active trip
        _listenToActiveTrip(driverId);
        
        // Listen to notifications if not already listening
        if (_notificationSubscription == null) {
          _notificationSubscription = _notificationService.streamNotifications(driverId).listen((notifications) {
            if (state is DriverLoaded) {
              final prev = state as DriverLoaded;
              emit(prev.copyWith(notifications: notifications));
            }
          });
        }
      },
    ...
    _walletSubscription = _driverService.streamWalletBalance(driverId).listen((balance) {
      if (state is DriverLoaded) {
        final prev = state as DriverLoaded;
        final updatedProfile = prev.profile.copyWith(walletBalance: balance);
        emit(prev.copyWith(profile: updatedProfile));
        
        // Re-evaluate request listening based on new balance
        if (updatedProfile.canReceiveRequests) {
          _listenToRequests();
        } else {
          _requestsSubscription?.cancel();
        }

        if (updatedProfile.isLowBalance && !updatedProfile.isBlocked) {
          _notificationService.notifyLowBalance(driverId, balance);
        }
      }
    });

    // Load history
    _loadTripHistory(driverId);
  }
  ...
  Future<void> _startLocationTracking(String driverId) async {
    ...
    if (position != null) {
      ...
      await _locationService.updateDriverLocation(
          driverId, position.latitude, position.longitude);
      // Re-listen to requests with the new real location
      _listenToRequests();
    }
    ...
    _locationSubscription = _locationService
        .streamAndSyncDriverLocation(driverId)
        .listen((pos) {
      _driverLat = pos.latitude;
      _driverLng = pos.longitude;
      // Re-subscribe to requests with updated location
      _listenToRequests();
      // Update route if there's an active trip
      if (state is DriverLoaded && (state as DriverLoaded).activeTrip != null) {
        _updateRoute();
      }
    });
  }
  ...
  Future<void> _updateRoute() async {
    if (state is! DriverLoaded) return;
    final currentState = state as DriverLoaded;
    final trip = currentState.activeTrip;
    if (trip == null || _driverLat == null || _driverLng == null) return;

    LatLng endPoint;
    
    if (trip.status == 'in_progress') {
      if (trip.destinationLat != null && trip.destinationLng != null) {
        endPoint = LatLng(trip.destinationLat!, trip.destinationLng!);
      } else {
        return; // Cannot route without destination coordinates
      }
    } else {
      if (trip.pickupLat != null && trip.pickupLng != null) {
        endPoint = LatLng(trip.pickupLat!, trip.pickupLng!);
      } else {
        return; // Cannot route without pickup coordinates
      }
    }
    ...
    if (state is DriverLoaded) {
      emit((state as DriverLoaded).copyWith(routePoints: points));
    }
  }
  ...
  void _listenToRequests() {
    _requestsSubscription?.cancel();
    _requestsSubscription = _tripService.streamNearbyRequests(
      ...
      (requests) {
        if (state is DriverLoaded) {
          final prev = state as DriverLoaded;
          // Check for new requests to show local notification
          final oldIds = prev.nearbyRequests.map((e) => e.id).toSet();
          for (var r in requests) {
            if (!oldIds.contains(r.id)) {
              _notificationService.notifyNewRequest(prev.profile.id, r.clientName);
              PushNotificationService.showLocalNotification(
                title: 'Nouvelle commande',
                body: '${r.clientName} vous a envoyé une demande de course.',
              );
            }
          }
          emit(prev.copyWith(nearbyRequests: requests));
        }
      },
      onError: (_) {},
    );
  ...
  // ─── Active Trip Listening ─────────────────────────────────────

  void _listenToActiveTrip(String driverId) {
    _activeTripSubscription?.cancel();
    _activeTripSubscription =
        _tripService.streamActiveTrip(driverId).listen(
      (trip) {
        if (state is DriverLoaded) {
          if (trip != null) {
            emit((state as DriverLoaded).copyWith(activeTrip: trip));
            _updateRoute();
          } else {
            emit((state as DriverLoaded)
                .copyWith(clearActiveTrip: true, clearRoute: true));
          }
        }
      },
      onError: (_) {},
  ...
  void _loadTripHistory(String driverId) {
    _historySubscription?.cancel();
    _historySubscription =
        _tripService.streamTripHistory(driverId).listen(
      (history) {
        if (state is DriverLoaded) {
          emit((state as DriverLoaded).copyWith(tripHistory: history));
        }
      },
      onError: (_) {},
    );
  ...
  Future<void> toggleStatus(String driverId, bool isOnline) async {
    try {
      if (isOnline) {
        // Check if blocked
        if (state is DriverLoaded &&
            (state as DriverLoaded).profile.isBlocked) {
          _emitTransientError('Compte bloqué. Rechargez votre solde.');
          return;
        }
        await _driverService.goOnline(driverId);
      } else {
        await _driverService.goOffline(driverId);
      }
    } catch (e) {
      _emitTransientError('Erreur de connexion');
    }
  }

  // ─── Accept Trip ───────────────────────────────────────────────

  Future<void> acceptTrip(String driverId, String tripId) async {
    try {
      String? clientName;
      String? clientPhone;
      ...
      if (state is DriverLoaded) {
        ...
        try {
          final trip = current.nearbyRequests.firstWhere((r) => r.id == tripId);
          clientName = trip.clientName;
          clientPhone = trip.clientPhone;
        } catch (_) {}
      }
      
      // Atomically lock the trip for this driver
      ...
      await _driverService.updateDriverStatus(driverId, 'busy');
    } catch (e) {
      final error = e.toString();
      if (error.contains('Solde insuffisant')) {
        _emitTransientError(error.replaceAll('Exception: ', ''));
      } else {
        _emitTransientError('Course déjà prise ou expirée');
      }
    }
  }
  ...
  void dismissTrip(String tripId) {
    if (state is DriverLoaded) {
      final current = state as DriverLoaded;
      final filtered =
          current.nearbyRequests.where((r) => r.id != tripId).toList();
      emit(current.copyWith(nearbyRequests: filtered));
    }
  }
  ...
  Future<void> arrivedAtPickup(String tripId) async {
    try {
      await _tripService.updateTripStatus(tripId, 'arrived');
    } catch (e) {
      _emitTransientError('Erreur de mise à jour');
    }
  }

  /// Start the trip (client picked up)
  Future<void> startTrip(String tripId, String driverId) async {
    try {
      await _tripService.updateTripStatus(tripId, 'in_progress');
      await _driverService.updateDriverStatus(driverId, 'on_trip');
    } catch (e) {
      _emitTransientError('Erreur de mise à jour');
    }
  }

  /// End/complete the trip
  Future<void> endTrip(
      String tripId, String driverId, double fare, double commissionRate) async {
    try {
      if (state is DriverLoaded) {
        emit((state as DriverLoaded).copyWith(clearActiveTrip: true, clearRoute: true));
      }
      final commission = fare * commissionRate;
      final netEarning = fare - commission;

      // Complete the trip in Firestore (now handles wallet deduction and trip count atomically)
      await _tripService.completeTrip(tripId, fare, commission);

      // Set back to online
      await _driverService.goOnline(driverId);

      // Notify the driver of their earning (what they keep after commission)
      _notificationService.notifyTripCompleted(driverId, netEarning);

      // Check low balance
      final blocked =
          await _driverService.checkAndBlockIfLowBalance(driverId);
      if (blocked) {
        _notificationService
            .notifyLowBalance(driverId, 0); // approximate, will refresh via stream
      }
    } catch (e) {
      _emitTransientError('Erreur lors de la finalisation');
    }
  }

  /// Cancel a trip
  Future<void> cancelTrip(String tripId, String driverId) async {
    try {
      if (state is DriverLoaded) {
        emit((state as DriverLoaded).copyWith(clearActiveTrip: true, clearRoute: true));
      }
      ...
      await _driverService.goOnline(driverId);
      _notificationService.notifyTripCancelled(driverId, tripId);
    } catch (e) {
      _emitTransientError('Erreur d\'annulation');
    }
  }
  ...
  void markAllNotificationsRead() {
    if (state is DriverLoaded) {
      final current = state as DriverLoaded;
      for (var n in current.notifications) {
        if (!n.isRead) {
  ...
  void clearNotifications() {
    // Optionally implement deleting notifications from Firestore here
  }

  void clearError() {
    if (state is DriverLoaded) {
      emit((state as DriverLoaded).copyWith(clearError: true));
    }
  }

  // ─── Profile Update ────────────────────────────────────────────

  Future<void> updateProfile(String driverId, String name, String phone) async {
    try {
      await _driverService.updateDriverProfile(driverId, name, phone);
    } catch (e) {
      final msg = e.toString().replaceAll('Exception: ', '');
      _emitTransientError(msg);
      throw Exception(msg);
    }
  }
  ...
  void _emitTransientError(String message) {
    if (state is DriverLoaded) {
      emit((state as DriverLoaded).copyWith(errorMessage: message));
    }
  }
...
class DriverModel extends Equatable {
  ...
  bool get isLowBalance => walletBalance < 50.0;
  bool get canReceiveRequests => status == 'online' && !isBlocked && !isLowBalance;
  bool get hasLocation => location != null || (latitude != null && longitude != null);

  static String normalizeStatus(String? rawStatus, bool? rawIsOnline) {
    final status = (rawStatus ?? '').toLowerCase().trim();
    if (status == 'busy' || status == 'on_trip') return 'busy';
    if (status == 'online' || status == 'available' || status == 'active') return 'online';
    if (rawIsOnline == true) return 'online';
    return 'offline';
  }
  ...
  factory DriverModel.fromJson(Map<String, dynamic> json, String id) {
    ...
    final rawIsOnline = (json['isOnline'] as bool?) ?? (json['is_online'] as bool?);
    final normalizedStatus = normalizeStatus(json['status'] as String?, rawIsOnline);
    ...
    return DriverModel(
      id: id,
      userId: (json['userId'] ?? json['user_id'] ?? id).toString(),
      ...
      location: location,
      licensePlate: (json['licensePlate'] ?? json['license_plate'] ?? '').toString(),
      commissionRate: _parseDouble(json['commissionRate'], 0.1),
      walletBalance: _parseDouble(json['walletBalance'], 0.0),
      createdAt: createdAt,
      latitude: lat,
      longitude: lng,
    );
  ...
  Map<String, dynamic> toJson() {
    final normalizedStatus = normalizeStatus(status, isOnline);
    final online = normalizedStatus == 'online';
    return {
      'id': id,
      'userId': userId.isEmpty ? id : userId,
      'name': name,
      'phone': phone,
      'rating': rating,
      ...
      // Backward compatibility fields (non-destructive migration)
      'user_id': userId.isEmpty ? id : userId,
      'is_online': online,
  ...
  DriverModel copyWith({
    ...
    final normalizedStatus = normalizeStatus(status ?? this.status, isOnline ?? this.isOnline);
    return DriverModel(
      id: id ?? this.id,
...
class NotificationModel extends Equatable {
  ...
  final String receiverRole; // e.g., 'driver', 'client'
  ...
  // Factory constructor to parse from Firestore (assuming standard fields)
  ...
  @override
  List<Object?> get props => [id, receiverId, receiverRole, type, title, message, timestamp, isRead];
}
...
class TripModel extends Equatable {
  final String id;
  final String userId;       // user_id from Firestore
  final String clientName;   // Fetched from users collection or empty
  final String clientPhone;  // Fetched from users collection or empty
  final double distance;
  final double estimatedPrice; // 'price' in Firestore
  final String pickupLocation;      // Display name (reverse geocoded or formatted)
  final String destinationLocation;  // Display name (reverse geocoded or formatted)
  ...
  final double? pickupLat;   // From GeoPoint pickup_location
  final double? pickupLng;
  final double? destinationLat; // From GeoPoint dest_location
  ...
  bool get hasDestCoords => destinationLat != null && destinationLng != null;
  double get netEarning => (fare ?? estimatedPrice) - (commission ?? 0);
  ...
  /// Format GeoPoint coordinates as a readable string
  static String _formatGeoPoint(double lat, double lng) {
  ...
  factory TripModel.fromJson(Map<String, dynamic> json, String id) {
    // Handle GeoPoint for pickup location
    double? pickupLat;
    double? pickupLng;
    String pickupLocationStr = '';
    final pickupGeo = json['pickup_location'];
    if (pickupGeo is GeoPoint) {
      pickupLat = pickupGeo.latitude;
      pickupLng = pickupGeo.longitude;
      pickupLocationStr = _formatGeoPoint(pickupGeo.latitude, pickupGeo.longitude);
    } else if (pickupGeo is String) {
      pickupLocationStr = pickupGeo;
    }
    // Also check for separate lat/lng fields (backward compatibility)
    pickupLat ??= (json['pickupLat'] as num?)?.toDouble();
    pickupLng ??= (json['pickupLng'] as num?)?.toDouble();

    // Handle GeoPoint for destination location
    double? destLat;
    double? destLng;
    String destLocationStr = '';
    final destGeo = json['dest_location'];
    if (destGeo is GeoPoint) {
      destLat = destGeo.latitude;
      destLng = destGeo.longitude;
      destLocationStr = _formatGeoPoint(destGeo.latitude, destGeo.longitude);
    } else if (destGeo is String) {
      destLocationStr = destGeo;
    }
    // Also check for separate lat/lng fields (backward compatibility)
    destLat ??= (json['destinationLat'] as num?)?.toDouble();
    destLng ??= (json['destinationLng'] as num?)?.toDouble();

    // Fallback to old field names for location strings
    if (pickupLocationStr.isEmpty) {
      pickupLocationStr = json['pickupLocation'] as String? ?? '';
    }
    if (destLocationStr.isEmpty) {
      destLocationStr = json['destinationLocation'] as String? ?? '';
    }

    // Calculate distance from coordinates if not provided
    double distance = (json['distance'] as num?)?.toDouble() ?? 0.0;
    if (distance == 0.0 && pickupLat != null && pickupLng != null && destLat != null && destLng != null) {
      distance = _calculateDistance(pickupLat, pickupLng, destLat, destLng);
    }

    // Handle price (Firestore uses 'price', old code used 'estimatedPrice')
    final price = (json['price'] as num?)?.toDouble() ??
        (json['estimatedPrice'] as num?)?.toDouble() ?? 0.0;

    // Handle created_at (could be Timestamp or String)
    DateTime? createdAt;
    ...
    // Handle accepted_at
    DateTime? acceptedAt;
    final acceptedAtRaw = json['accepted_at'] ?? json['acceptedAt'];
    if (acceptedAtRaw is Timestamp) {
      acceptedAt = acceptedAtRaw.toDate();
    } else if (acceptedAtRaw is String) {
      acceptedAt = DateTime.tryParse(acceptedAtRaw);
    }
    ...
    return TripModel(
      ...
      destinationLocation: destLocationStr,
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'pickup_location': (pickupLat != null && pickupLng != null)
          ...
          : null,
      'dest_location': (destinationLat != null && destinationLng != null)
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: Text(AppLocalizations.translate('about', lang),
        ...
        child: Column(
          children: [
              ...
              child: Column(
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.pedal_bike, color: Colors.black, size: 50),
                  ),
                  ...
                  Text(
                    AppLocalizations.translate('app_tagline', lang),
                  ...
                  Container(
                    ...
                    child: const Text(
                      'Version 1.0.0',
                      style: TextStyle(color: Color(0xFFF5D142), fontSize: 13, fontWeight: FontWeight.bold),
                    ),
                  ),
                ],
              ),
            ...
            _infoCard(
              ...
              title: AppLocalizations.translate('about_app', lang),
              content: AppLocalizations.translate('about_app_desc', lang),
            ...
            _infoCard(
              ...
              title: AppLocalizations.translate('our_mission', lang),
              content: AppLocalizations.translate('our_mission_desc', lang),
              color: goldColor,
            ),
            const SizedBox(height: 16),
            // Stats row
            Row(
              children: [
                Expanded(child: _statCard('100+', AppLocalizations.translate('drivers', lang), Icons.people_outline, Colors.greenAccent)),
                const SizedBox(width: 12),
                Expanded(child: _statCard('1000+', AppLocalizations.translate('trips_done', lang), Icons.pedal_bike, goldColor)),
                const SizedBox(width: 12),
                Expanded(child: _statCard('4.8 ⭐', AppLocalizations.translate('rating_label', lang), Icons.star_outline, Colors.orangeAccent)),
              ],
            ),
            const SizedBox(height: 28),
            // Legal
            _sectionHeader(Icons.gavel_outlined, AppLocalizations.translate('legal', lang), Colors.grey),
            const SizedBox(height: 12),
            _legalTile(Icons.description_outlined, AppLocalizations.translate('terms_of_service', lang)),
            _legalTile(Icons.privacy_tip_outlined, AppLocalizations.translate('privacy_policy', lang)),
            _legalTile(Icons.copyright_outlined, '©️ 2025 Tawseel. ${AppLocalizations.translate('all_rights_reserved', lang)}'),
            const SizedBox(height: 20),
            // Made with love
            Text(
              AppLocalizations.translate('made_with_love', lang),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          Text(content, style: TextStyle(color: Colors.grey.shade400, fontSize: 13, height: 1.6)),
        ],
      ),
    );
  }
